...
class CSICapture(Thread):
    def __init__(self, camera_num=0, res=None, network_table=None, exp=None, frate=None):
        ...
        if frate is not None:
            ...
            self.framerate = configs['CSIframerate']
    ...
    # Read
    @property
    # Give most recent frame
    def frame(self):
        self._new_frame = False
        return self._frame
    @property
    # Check if we have new frame
    def new_frame(self):
        return self._new_frame
    # Write
    @new_frame.setter
    # Announce if new frame available
    ...
    @property
    def resolution(self):            
        if self.camera_open: return self.camera.resolution                            
        else: return float("NaN")
    @property
    def width(self):                 
        if self.camera_open: return self.camera.resolution[0]
        else: return float("NaN")
    @property
    def height(self):                
        if self.camera_open: return self.camera.resolution[1]                         
        else: return float("NaN")
    @property
    def framerate(self):             
        if self.camera_open: return self.camera.framerate+self.camera.framerate_delta 
        else: return float("NaN")
    @property
    def exposure(self):              
        if self.camera_open: return self.camera.exposure_speed                        
        else: return float("NaN")
    # Write
    @resolution.setter
    ...
    @property
    def awb_mode(self):              
        if self.camera_open: return self.camera.awb_mode               
        else: return float('NaN')
    @property
    def awb_gains(self):             
        if self.camera_open: return self.camera.awb_gains              
        else: return float('NaN')
    @property
    def analog_gain(self):           
        if self.camera_open: return self.camera.analog_gain           
        else: return float("NaN")
    @property
    def digital_gain(self):          
        if self.camera_open: return self.camera.digital_gain           
        else: return float("NaN")
    # Write
    @awb_mode.setter
    ...
    @property
    def brightness(self):            
        if self.camera_open: return self.camera.brightness             
        else: return float('NaN')
    @property
    def iso(self):                   
        if self.camera_open: return self.camera.iso                    
        else: return float("NaN")
    @property
    def exposure_mode(self):         
        if self.camera_open: return self.camera.exposure_mode          
        else: return float("NaN")
    @property
    def exposure_compensation(self): 
        if self.camera_open: return self.camera.exposure_compensation  
        else: return float("NaN")
    @property
    def drc_strength(self):          
        if self.camera_open: return self.camera.drc_strength           
        else: return float('NaN')
    @property
    def contrast(self):              
        if self.camera_open: return self.camera.contrast               
        else: return float('NaN')
    # Write
    @brightness.setter
    ...
    @property
    def flash_mode(self):            
        if self.camera_open: return self.camera.flash_mode             
        else: return float('NaN')
    @property
    def clock_mode(self):            
        if self.camera_open: return self.camera.clock_mode             
        else: return float('NaN')
    @property
    def sharpness(self):             
        if self.camera_open: return self.camera.sharpness              
        else: return float('NaN')
    @property
    def color_effects(self):         
        if self.camera_open: return self.camera.color_effects           
        else: return float('NaN')
    @property
    def image_effect(self):          
        if self.camera_open: return self.camera.image_effect           
        else: return float('NaN')
    @property
    def image_denoise(self):         
        if self.camera_open: return self.camera.image_denoise          
        else: return float('NaN')
    @property
    def video_denoise(self):         
        if self.camera_open: return self.camera.video_denoise          
        else: return float('NaN')
    @property
    def video_stabilization(self):   
        if self.camera_open: return self.camera.video_stabilization    
        else: return float('NaN')
    # Write
    @flash_mode.setter
